...
class Graph:
	...
	def get_edges(self):
		mask = np.tile(self.mask, (len(self.mask),1))
		mask2d = mask & mask.T
		edges = np.where(np.triu(self._adj_matrix & mask2d))
		return zip(edges[0], edges[1])

	def too_expensive_edges(self, k):
		mask = np.tile(self.mask, (len(self.mask),1))
		mask2d = mask & mask.T
		edges = np.where((np.triu(self.weights > k) & np.triu(self._adj_matrix & mask2d)))
		return list(zip(edges[0], edges[1]))

	def get_neighbors(self, v):
		return np.where(self._adj_matrix[v] & self.mask)[0]

	def common_neighbors(self, v, w):
		return np.where((self._adj_matrix[v] & self._adj_matrix[w]) & self.mask)[0]

	def all_neighbors(self, v, w):
		union = (self._adj_matrix[v] | self._adj_matrix[w]) & self.mask
	...
	def single_neighbors(self, v, w):
		...
		single_neighbors = (self._adj_matrix[v] & np.invert(self._adj_matrix[w])) & self.mask
	...
	def get_p3s(self):
		...
		# for u, v in self.get_edges():
		# 	for w in self.single_neighbors(u, v):
		# 		p3s.add(self.P3_2(w, u, v, self.weights))
		# 	for w in self.single_neighbors(v, u):
		# 		p3s.add(self.P3_2(u, v, w, self.weights))
		return p3s
	...
	# def merge_cost(self, u, v):
	# 	sign_mask = (np.sign(self.weights[u]) != np.sign(self.weights[v])) & self.mask
	# 	return np.sum(np.minimum(np.abs(self.weights[u][sign_mask]), np.abs(self.weights[v][sign_mask])))
	def merge_cost(self, u, v):
		sign_mask = (self.sign(self.weights[u]) != self.sign(self.weights[v])) & self.mask
		# sign_mask = (np.sign(self.weights[u]) != np.sign(self.weights[v])) & self.mask
		return np.sum(np.minimum(np.abs(self.weights[u][sign_mask]), np.abs(self.weights[v][sign_mask])))
	...
	def connected_components(self):
		...
		not_visited = copy.copy(self.mask)
